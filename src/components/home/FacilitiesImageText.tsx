...
export const FacilitiesImageText = ({
  ...
  return (
    <div className="absolute right-0 bottom-0 left-0 bg-linear-to-t from-black/80 via-black/40 to-transparent p-5 text-white/90">
      <h3 className="text-sm font-semibold tracking-wide">{tTitle(title)}</h3>
      <p className="mt-1 mb-2.5 text-xs text-white/80 lg:mb-0">{tDesc(desc)}</p>
    </div>
  );
};
